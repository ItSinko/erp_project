...
<div class="row">
  <div class="col-lg-12">
    <div class="col-lg-12">
      <form method="post" action="/penawaran_ecom/aksi_tambah">
        {{ csrf_field() }}
        <div class="card">
                ...
                <div class="col-lg-12">
                  <div class="form-horizontal">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-sm-4">
                        <select class="form-control @error('ecommerce_id') is-invalid @enderror select2" name="ecommerce_id">
                          <option value="">Pilih Status</option>
                        </select>
                        @if($errors->has('ecommerce_id'))
                        <div class="text-danger">
                          {{ $errors->first('ecommerce_id')}}
                        </div>
                        @endif
                      </div>
                    </div>
...
<script>
  $(document).ready(function() {
    $('select[name="customer_id"]').on('change', function() {
      ...
      if (customer_id) {
        $.ajax({
          url: '/penawaran_ecom/data/' + customer_id,
          type: "GET",
          dataType: "json",
          success: function(data) {

            jQuery('select[name="ecommerce_id"]').empty();
            $('select[name="ecommerce_id"]').append('<option value=""></option>');
            $.each(data, function(key, value) {
              console.log(value);
              $('select[name="ecommerce_id"]').append('<option value="' + value['id'] + '">' + value['order_id'] + '</option>');
            });
          },
        });
      } else {
